...
const NoteCard = ({ note, onAction }) => {
  const isPaid = note.isPaid || false;
  const targetClass = note.targetClass || note.classTarget || note.class || 'All Classes';
  ...
  const handleAction = () => {
    if (onAction) {
      ...
      alert("Please open this note from your Dashboard Vault!");
    }
  };
    ...
    <div className="flex flex-col justify-between bg-zinc-900/80 border border-zinc-800 hover:border-cyan-500/40 p-5 sm:p-6 rounded-2xl transition-all duration-300 hover:-translate-y-1 shadow-xl relative overflow-hidden group">
      ...
      <div>
        ...
        <p className="text-xs text-zinc-400 leading-relaxed mb-4 line-clamp-2">
          {description || "Handwritten Chapterwise Notes & Important Questions."}
        </p>
      ...
      <div className="pt-4 border-t border-zinc-800/80 flex items-center justify-between">
        <div>
          {isPaid ? (
            <div className="flex items-baseline gap-1.5">
              <span className="text-xl font-black text-white">{note.price ? `₹${note.price}` : '₹99'}</span>
              {note.originalPrice && (
                <span className="text-xs text-zinc-500 line-through">₹{note.originalPrice}</span>
              )}
            </div>
          ) : (
            <span className="text-xs font-bold text-emerald-400">FREE ACCESS</span>
          )}
        </div>
        ...
        >
          {isPaid ? "Unlock Notes 🔓" : "Read In-App 📄"}
        </Button>
      </div>
